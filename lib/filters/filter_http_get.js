    ...
    this.on('input', function(data) {
      if (this.processMessage(data)) {
        try {
          ...
          if( data.http_secure){
            request = https;
            uri = this.replaceByFields(data,'https://' + url);
          } else {
            request = http;
            uri = this.replaceByFields(data,'http://' + url);
          }
          logger.debug('Http Get on:'+uri);
          request.get( uri, function(res){
                if (res.statusCode < 200 || res.statusCode > 299) {
                  logger.error('Http Get response status code:'+res.statusCode);
                }
              data.message = '';
